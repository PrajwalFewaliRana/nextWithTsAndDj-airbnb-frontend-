  ...
  const performBooking = async ()=>{
    ...
    if (userId){
      if (dateRange.startDate && dateRange.endDate){
        ...
        const response = await apiService.postForm(`/api/properties/${property.id}/book/`,formData)
  ...
  return (
    <aside className="mt-6 p-6 col-span-2 rounded-xl border border-gray-300 shadow-xl">
      ...
      <div className="mb-6 p-3 border border-gray-400 rounded-xl">
        <label htmlFor="" className="mb-2 font-bold block text-xs">
          Guests
        </label>
        <select value={guests} onChange={(e)=>setGuests(e.target.value)} name="" id="" className="w-full -ml-1 text-xm">
          {guestsRange.map(number=>(
            <option value={number} key={number}>{number}</option>
          ))}
        </select>
      </div>
      <div onClick={performBooking} className="w-full mb-6 py-6 text-center text-white  airbnb rounded-xl">
        Book
      </div>
